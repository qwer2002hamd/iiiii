...
async def markup_panel(client, CallbackQuery: CallbackQuery, _):
    ...
    buttons = panel_markup_1(_, videoid, chat_id, user_id, query, query_type, channel, fplay)
    try:
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if state == "Forw":
        if pages == 0:
            buttons = panel_markup_2(_, videoid, chat_id, user_id, query, query_type, channel, fplay)
        if pages == 2:
            buttons = panel_markup_1(_, videoid, chat_id, user_id, query, query_type, channel, fplay)
        if pages == 1:
            buttons = panel_markup_3(_, videoid, chat_id, user_id, query, query_type, channel, fplay)
    if state == "Back":
        if pages == 2:
            buttons = panel_markup_2(_, videoid, chat_id, user_id, query, query_type, channel, fplay)
        if pages == 1:
