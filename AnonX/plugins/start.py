...
async def start_comm(client, message: Message, _):
    ...
    if len(message.text.split()) > 1:
        ...
        if name[0:3] == "inf":
            ...
            searched_text = f"""
🔍__**مـعلومات حـول الـفيديو**__

❇️**الـوصف:** {title}

⏳**الـوقت:** {duration} Mins
👀**الآراء:** `{views}`
⏰**مـدة الـنشر:** {published}
🎥**الـقناة:** {channel}
📎**رابط الـقناة:** [Visit From Here]({channellink})
🔗**رابط الـفيديو:** [Link]({link})

⚡️ __تـم الـبحث بـواسطة {config.MUSIC_BOT_NAME}__"""
            key = InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text="‹ مـشاهده ›", url=f"{link}"
                        ),
                        InlineKeyboardButton(
        ...
        if await is_on_off(config.LOG):
            ...
            return await app.send_message(
                config.LOG_GROUP_ID,
                f"{message.from_user.mention} ارسل ستارت للبوت !\n\n**ايديه :** {sender_id}\n**اسم حسابه :** {sender_name}\n**يوزره :** @{message.from_user.username}",
            )
